...
//添加任务
int pool_add_worker (int fd,HANDLER handler)
{
    //上锁
    pthread_mutex_lock (&(pool->queue_lock));
    enLqueue(pool->lqueue,fd,handler);
...
void *thread_routine (void *arg)
{
    ...
    {  
        ...
        assert (pool->lq->front->next == NULL);   //如果head为空  终止

        //等待队列长度减去1，并取出链表中的头元素
        pool->cur_queue_size--;  
        //出队
        linklist *temp = deLqueue(pool->lqueue);
        //解锁
        pthread_mutex_unlock (&(pool->queue_lock));

        //调用函数指针，执行任务
        temp->handler(temp->fd);  
        free (temp);  //销毁这个节点
        worker = NULL;  
    }
}
